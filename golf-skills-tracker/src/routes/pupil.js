...
router.post('/challenges/:id/scores', async (req, res, next) => {
  try {
    ...
    await query('INSERT INTO scores (pupil_id, challenge_id, value) VALUES ($1,$2,$3)', [
      req.session.pupilId,
      req.params.id,
      value,
    ]);
...
router.get('/history/:challengeId', async (req, res, next) => {
  try {
    ...
    const { rows: scores } = await query(
      `SELECT id, value, created_at, updated_at FROM scores
...
router.post('/scores/:scoreId/edit', async (req, res, next) => {
  try {
    ...
    if (!Number.isNaN(value)) {
      await query('UPDATE scores SET value = $1, updated_at = now() WHERE id = $2 AND pupil_id = $3', [
        value,
        req.params.scoreId,
        req.session.pupilId,
      ]);
