...
router.get('/history/:challengeId', async (req, res, next) => {
    ...
    // Chart needs oldest-to-newest order; the table above shows newest-first.
    const chartPoints = scores
        .slice()
        .reverse()
        .map((s) => ({ value: s.value, date: s.created_at }));
        // Neutralise "</script>" so the JSON can be embedded safely inside a <script> tag.
